...
static int 	dsnprintf_t(char_t **s, int size, char_t *fmt, va_list arg,
				int msize);
static int	strnlen_t(char_t *s, unsigned int n);
static void	put_char_t(strbuf_t *buf, char_t c);
static void	put_string_t(strbuf_t *buf, char_t *s, int len,
				int width, int prec, enum flag f);
static void	put_ulong_t(strbuf_t *buf, unsigned long int value, int base,
...
int fmtAlloc(char_t **s, int n, char_t *fmt, ...)
{
	...
	*s = NULL;
	va_start(ap, fmt);
	result = dsnprintf_t(s, n, fmt, ap, 0);
	va_end(ap);
	return result;
}
...
int fmtRealloc(char_t **s, int n, int msize, char_t *fmt, ...)
{
	...
	va_start(ap, fmt);
	result = dsnprintf_t(s, n, fmt, ap, msize);
	va_end(ap);
	return result;
}
...
int fmtValloc(char_t **s, int n, char_t *fmt, va_list arg)
{
	...
	*s = NULL;
	return dsnprintf_t(s, n, fmt, arg, 0);
}
...
static int dsnprintf_t(char_t **s, int size, char_t *fmt, va_list arg, int msize)
{
	strbuf_t	buf;
	char_t		c;
	...
	while ((c = *fmt++) != '\0') {
		if (c != '%' || (c = *fmt++) == '%') {
			...
			if (c == 'd' || c == 'i') {
				...
				if (value >= 0) {
					if (f & flag_plus) {
						put_ulong_t(&buf, value, 10, 0, T("+"), width, prec, f);
					} else if (f & flag_space) {
						put_ulong_t(&buf, value, 10, 0, T(" "), width, prec, f);
					} else {
						put_ulong_t(&buf, value, 10, 0, NULL, width, prec, f);
					}
				} else {
					put_ulong_t(&buf, -value, 10, 0, T("-"), width, prec, f);
				}
				...
				if (c == 'o') {
					if (f & flag_hash && value != 0) {
						put_ulong_t(&buf, value, 8, 0, T("0"), width, prec, f);
					} else {
						put_ulong_t(&buf, value, 8, 0, NULL, width, prec, f);
					}
				} else if (c == 'u') {
					put_ulong_t(&buf, value, 10, 0, NULL, width, prec, f);
				} else {
					if (f & flag_hash && value != 0) {
						if (c == 'x') {
							put_ulong_t(&buf, value, 16, 0, T("0x"), width, 
								prec, f);
						} else {
							put_ulong_t(&buf, value, 16, 1, T("0X"), width, 
								prec, f);
						}
					} else {
                  /* 04 Apr 02 BgP -- changed so that %X correctly outputs
                   * uppercase hex digits when requested.
						put_ulong_t(&buf, value, 16, 0, NULL, width, prec, f);
                   */
						put_ulong_t(&buf, value, 16, ('X' == c) , NULL, width, prec, f);
					}
				}

			} else if (c == 'c') {
				char_t value = va_arg(arg, int);
				put_char_t(&buf, value);

			} else if (c == 's' || c == 'S') {
				char_t *value = va_arg(arg, char_t *);
				if (value == NULL) {
					put_string_t(&buf, T("(null)"), -1, width, prec, f);
				} else if (f & flag_hash) {
					put_string_t(&buf,
						value + 1, (char_t) *value, width, prec, f);
				} else {
					put_string_t(&buf, value, -1, width, prec, f);
				}
			} else if (c == 'p') {
...
static void put_string_t(strbuf_t *buf, char_t *s, int len, int width,
		int prec, enum flag f)
{
	int		i;

	if (len < 0) { 
		len = strnlen_t(s, prec >= 0 ? prec : ULONG_MAX); 
		...
		int upper, char_t *prefix, int width, int prec, enum flag f)
{
	...
	width -= zeros + len;
	if (prefix != NULL) { 
		width -= strnlen_t(prefix, ULONG_MAX); 
	}
	if (!(f & flag_minus)) {
		if (f & flag_zero) {
			for (i = 0; i < width; ++i) { 
	...
	if (prefix != NULL) { 
		put_string_t(buf, prefix, -1, 0, -1, flag_none); 
	}
	for (i = 0; i < zeros; ++i) { 
		put_char_t(buf, '0'); 
	}
	for ( ; x > 0; x /= base) {
		...
		put_char_t(buf, (char) ((digit < 10 ? '0' : (upper ? 'A' : 'a') - 10) +
			digit));
	}
	if (f & flag_minus) {
		for (i = 0; i < width; ++i) { 
