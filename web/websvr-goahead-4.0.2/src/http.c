...
#define UserPW "/home/xueqian/goahead/config/UserPW"
static void uploadTest(Webs *wp)
{
	...
	if (scaselessmatch(wp->method, "POST")) {
		for (s = hashFirst(wp->files); s; s = hashNext(wp->files, s)) {
			...
			websWrite(wp, "TYPE=%s\r\n", up->contentType);
			websWrite(wp, "SIZE=%d\r\n", up->size);
			if(strcmp("system.tar",up->clientFilename)){
				printf("文件名错误\n");
				websWrite(wp, "文件非法，请检查！");
				websDone(wp);
				return;
			}	
			//upfile = sfmt("%s/tmp/%s", websGetDocuments(), up->clientFilename);//文件存放的地方
			upfile = sfmt("%s/%s", uploadDir, up->clientFilename);//文件存放的地方
...
#endif
	websDefineAction("test",testFun);
	websDefineAction("uploadTest",uploadTest);

/*Account*/
	websDefineAction("settingPW", settingPW);
#if 1
...
/*settingLAN*/
	websDefineAction("settingLAN", settingLAN);
	websDefineAction("queryLAN", queryLAN);

/*settingDtu*/
	websDefineAction("checkDtuEnable", checkDtuEnable);
	websDefineAction("getDtuLog", getDtuLog);
	websDefineAction("startDtu", startDtu);
	websDefineAction("cancelDtu", cancelDtu);
	websDefineAction("settingDtu", settingDtu);
	websDefineAction("queryDtu", queryDtu);

/*settingSecurity*/
	websDefineAction("checkSecurityEnable", checkSecurityEnable);
	websDefineAction("startSecurity", startSecurity);
	websDefineAction("cancelSecurity", cancelSecurity);
...
/*Version*/
	websDefineAction("queryVer", queryVer);
#endif
/*Runningstatus*/
	websDefineAction("sysInfo", sysInfo);
	websDefineAction("deviceInfo", deviceInfo);
	websDefineAction("WANStatus", WANStatus);
	websDefineAction("LANStatus", LANStatus);
...
PUBLIC int websListen(cchar *endpoint)
{
    ...
    logmsg(2, "Started %s://%s:%d,AUTH %d", secure ? "https" : "http", ipaddr, port,ME_GOAHEAD_AUTH);
